...
class TestOctant:
    ...
    def test_distance_to(self):
        ...
        # For distance = √hamming, octants must be at (±0.5, ±0.5, ±0.5)
        octants = [
    ...
    def test_distance_categories(self):
        ...
        # Create octants at unit cube corners (size=1, positions at ±0.5)
        o0 = Octant(0, position=(-0.5, -0.5, -0.5), size=1.0)
        o1 = Octant(1, position=(+0.5, -0.5, -0.5), size=1.0)  # Edge
        o3 = Octant(3, position=(+0.5, +0.5, -0.5), size=1.0)  # Face diagonal
        o7 = Octant(7, position=(+0.5, +0.5, +0.5), size=1.0)  # Space diagonal
...
class TestGeometricInvariants:
    ...
    def test_distance_hamming_relationship(self):
        ...
        """
        # Create octants at unit cube corners (±0.5 for unit distances)
        octants = [
            Octant(i, position=tuple(s * 0.5 for s in OCTANT_SIGNS[i]), size=1.0)
            for i in range(8)
        ]
